      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project, index) => (
            ...
            >
              <CardHeader>
                <CardTitle className="hover-scale">{project.title}</CardTitle>
              ...
              <CardContent>
                <div className="flex flex-wrap gap-2 mb-4">
                  {project.tags.map((tag, tagIndex) => (
                    <span
                      key={tagIndex}
                      className="px-2 py-1 bg-secondary text-xs rounded-full transition-colors duration-300 hover:bg-primary/20"
                    >
                      {tag}
                    </span>
                  ))}
